...
[Sequence Game](https://codeforces.com/problemset/problem/1862/B) - Constructive
 We need to build
an array where $a_{i-1} \le a_i$. If the current element is smaller than
...
For two sub-groupsto share the same parity, their total sum must be even, as the sum of two
numbers with identical parity is always even. Since parity is only toggled by
